...
class EventController
{
    ...
    public function create()
    {
        ...
        $event = [
            'calendar_id' => isset($_GET['calendar_id']) ? $_GET['calendar_id'] : (!empty($calendars) ? $calendars[0]['calendar_id'] : ''),
            'category_id' => '',
            'title' => '',
            'body' => '',
    ...
    public function save()
    {
        ...
        try {
            ...
            error_log('Erreur de conversion des dates: ' . $e->getMessage());
            header('Content-Type: application/json');
        ...
        if (!isset($data['title']) || !isset($data['calendarId'])) {
            header('Content-Type: application/json');
    ...
    public function move()
    {
        ...
        error_log('Données de déplacement reçues: ' . json_encode($data));

        // Extraire correctement les dates (format simple ou objet TUI Calendar)
        $startDate = isset($data['start']['_date']) ? $data['start']['_date'] : (is_array($data['start']) ? json_encode($data['start']) : $data['start']);
        $endDate = isset($data['end']['_date']) ? $data['end']['_date'] : (is_array($data['end']) ? json_encode($data['end']) : $data['end']);

        // Log des dates extraites pour débogage
        error_log('Déplacement - Date de début extraite: ' . $startDate);
        error_log('Déplacement - Date de fin extraite: ' . $endDate);

        // Vérifier les données requises
        if (!isset($data['id']) || empty($startDate) || empty($endDate)) {
            header('Content-Type: application/json');
            echo json_encode(['success' => false, 'message' => 'Données incomplètes']);
            exit;
        }
        ...
        // Mettre à jour l'événement avec is_all_day
        $eventData = [
            'calendar_id' => $calendar_id,
        ...
        error_log('Mise à jour de l\'événement avec: ' . json_encode($eventData));
        $result = $this->eventModel->update($data['id'], $eventData);
            ...
            foreach ($events as $event) {
                ...
                if (!empty($event['category_id'])) {
                    try {
                        ...
                        if ($category) {
                            $categoryColor = $category['bg_color'];
                            $categoryTextColor = $category['color'];
                        }
                    } catch (Exception $e) {
