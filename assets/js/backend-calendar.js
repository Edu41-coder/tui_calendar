...
class CalendarBackend {
  ...
  saveEvent(eventData, callback) {
    ...
    $.ajax({
      ...
      success: (response) => {
        ...
        if (response.success) {
          this.alertSystem.success(
            response.message || "Événement enregistré avec succès"
          );
  ...
  moveEvent(moveData, callback) {
    ...
    if (startObj && endObj) {
      const startIsMidnight =
        startObj.getHours() === 0 && startObj.getMinutes() === 0;
      const endIs2359 = endObj.getHours() === 23 && endObj.getMinutes() === 59;

      if (startIsMidnight && endIs2359) {
        moveData.isAllDay = true;
        console.log(
          'Événement déplacé détecté comme "toute la journée" basé sur les heures'
        );
      }
    }
    ...
    $.ajax({
      ...
      success: (response) => {
        if (response.success) {
          this.alertSystem.success(
            response.message || "Événement déplacé avec succès"
          );
          if (callback && typeof callback === "function") {
            callback(null, response);
          }
  ...
  toggleCalendarVisibility(calendarId, visible, callback) {
    // Créer un FormData pour une meilleure compatibilité
    ...
    $.ajax({
      ...
      error: (xhr, status, error) => {
        ...
        this.alertSystem.error("Erreur de communication: " + (xhr.responseText || error));
        callback && callback(new Error(xhr.responseText || error), null);
      }
    });
  }
    ...
    fetch(url)
      ...
      .then((data) => {
        if (data.success) {
          ...
          callback &&
            callback(
              data.message || "Erreur lors de la récupération de l'événement",
              null
            );
        }
      })
      .catch((error) => {
        ...
        callback &&
          callback(
            "Erreur réseau lors de la récupération de l'événement",
            null
          );
      });
  }
